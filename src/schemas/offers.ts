...
export const createPropertyOfferSchema = z.object({
  offerAmount: z
    ...
    .max(999999999, "Offer amount is too high"),
  financing: z.enum(FINANCING_TYPES, {
    errorMap: () => ({ message: "Invalid financing type" }),
  }),
...
export type CreatePropertyOfferInput = z.infer<
  typeof createPropertyOfferSchema
>;

export const respondPropertyOfferSchema = z.object({
  status: z.enum(["accepted", "rejected", "countered"], {
    errorMap: () => ({
      message: "Status must be accepted, rejected, or countered",
    }),
  }),
...
export type RespondPropertyOfferInput = z.infer<
  typeof respondPropertyOfferSchema
>;
