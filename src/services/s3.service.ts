...
export function validateFileContent(buffer: Buffer, mimeType: string): { valid: boolean; error?: string } {
  ...
  if (!expected) {
    return { valid: false, error: `Cannot verify file content for type: ${mimeType}` };
  }
  for (let i = 0; i < expected.length; i++) {
    if (buffer[i] !== expected[i]) {
      return { valid: false, error: `File content does not match its declared type (${mimeType}). The file may be corrupted, renamed, or not a valid document.` };
    }
  }
  return { valid: true };
}
...
export function formatS3Error(err: any): string {
  ...
  if (msg.includes('AccessDenied')) return 'S3 access denied. Check AWS credentials and bucket permissions.';
  if (msg.includes('NoSuchBucket')) return 'S3 bucket not found. Check AWS_BUCKET configuration.';
  if (msg.includes('ExpiredToken')) return 'AWS credentials have expired. Please update ACCESS_KEY_ID and SECRET_ACCESS_KEY.';
  if (msg.includes('NetworkingError') || msg.includes('ENOTFOUND')) return 'Cannot reach AWS S3. Check network connectivity and AWS_REGION.';
  if (msg.includes('InvalidAccessKeyId')) return 'Invalid AWS Access Key ID. Check AWS_ACCESS_KEY_ID configuration.';
