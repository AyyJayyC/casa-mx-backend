...
export class LoggingService {
   ...
   */
  truncateObject(obj: any, maxSize = 10240): any {
    if (!obj) return obj;
   ...
   */
  async createDebugSession(data: any) {
   ...
   * End a debug session
   */
  async endDebugSession(sessionId: string) {
  ...
  async logError(data: any) {
    ...
    try {
      ...
      if (data.sessionId && data.sessionId !== "disabled") {
        await prisma.debugSession
          .update({
            where: { id: data.sessionId },
            data: { hasErrors: true },
          })
          .catch((err) =>
            logger.error({ err }, "Failed to update session errors flag"),
          );
      }
   ...
   */
  async getSessionWithLogs(sessionId: string) {
   ...
   */
  async resolveError(errorId: string, resolvedById: string, note: string) {
    if (!this.isEnabled) return null;
  ...
  async cleanupOldLogs(days = LOG_RETENTION_DAYS) {
    ...
    try {
      ...
      const [deletedActions, deletedErrors, deletedApis, deletedSessions] =
        await Promise.all([
