...
export async function setupLoggingMiddleware(fastify: FastifyInstance) {
  ...
  fastify.addHook("onResponse", async (request, reply) => {
    ...
    try {
      ...
      logger.error({ error, url: request.url }, "Failed to log response");
    }
  });
  ...
  fastify.addHook("onError", async (request, reply, error) => {
    ...
    await loggingService
      ...
      .catch((err) => logger.error({ err }, "Failed to log unhandled error"));
  });
}
...
function sanitizeHeaders(headers: Record<string, any>): Record<string, any> {
  ...
  if (sanitized["x-session-id"]) {
    sanitized["x-session-id"] =
      sanitized["x-session-id"].substring(0, 8) + "...";
  }
