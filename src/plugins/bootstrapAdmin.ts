...
export async function bootstrapAdmin(fastify: FastifyInstance) {
  ...
  try {
    ...
    if (!user) {
      fastify.log.info({ adminEmail }, 'ADMIN_EMAIL user not found yet — will bootstrap on next startup');
      return;
    }
    ...
    const role = await fastify.prisma.role.findUnique({ where: { name: 'admin' } });
    if (!role) {
