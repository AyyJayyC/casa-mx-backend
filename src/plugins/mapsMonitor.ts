...
const mapsMonitor: FastifyPluginAsync = async (fastify) => {
  const intervalMs = parseInt(
    process.env.MAPS_MONITOR_INTERVAL_MS || "300000",
    10,
  ); // default 5 minutes
  ...
  async function checkUsage() {
    try {
      ...
      for (const limit of limits) {
        const used = await fastify.prisma.apiUsageLog.count({
          where: {
        ...
        const percentage =
          limit.limitValue > 0
            ? Math.round((used / limit.limitValue) * 100)
            : 0;
        ...
        if (percentage >= limit.alertThreshold && percentage < 100) {
          ...
          const recent = await fastify.prisma.limitAlert.findFirst({
            where: {
              serviceType: limit.serviceType,
              alertType: "threshold",
              alertTimestamp: { gte: since },
              resolved: false,
            },
          });
          if (!recent) {
            await fastify.prisma.limitAlert.create({
              data: {
                serviceType: limit.serviceType,
                alertType: "threshold",
            ...
            fastify.log.warn(
              { service: limit.serviceType, used, limit: limit.limitValue },
              "Maps usage threshold reached",
            );
          }
        }
        ...
        if (used >= limit.limitValue) {
          if (limit.hardStop && limit.status !== "exceeded") {
            await fastify.prisma.usageLimit.update({
              where: { serviceType: limit.serviceType },
            ...
            fastify.log.error(
              { service: limit.serviceType, used, limit: limit.limitValue },
              "Maps usage limit exceeded - service hard-stopped",
            );
          }
        }
      }
