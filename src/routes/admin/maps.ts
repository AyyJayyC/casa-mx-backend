...
const updateLimitBodySchema = z
  ...
  .refine((value) => Object.keys(value).length > 0, {
    message:
      "At least one of limitValue, alertThreshold, or hardStop is required.",
  });
...
const adminMapsRoutes: FastifyPluginAsync = async (fastify, opts) => {
  ...
  fastify.patch(
    ...
    async (request, reply) => {
      ...
      const updated = await mapsService.updateLimit(
        parsedParams.data.serviceType,
        parsedBody.data,
      );
  ...
  fastify.patch(
    "/admin/maps/service/:serviceType/enable",
    { onRequest: [requireRole("admin")] },
    async (request, reply) => {
      const parsedParams = serviceTypeParamsSchema.safeParse(request.params);
      ...
      const updated = await mapsService.resumeService(
        parsedParams.data.serviceType,
      );
  ...
  fastify.patch(
    "/admin/maps/service/:serviceType/disable",
    { onRequest: [requireRole("admin")] },
    async (request, reply) => {
      const parsedParams = serviceTypeParamsSchema.safeParse(request.params);
      if (!parsedParams.success) {
      ...
      const updated = await mapsService.pauseService(
        parsedParams.data.serviceType,
      );
  ...
  fastify.get(
    ...
    async (request, reply) => {
      ...
      const usage = await mapsService.getUsage(
        parsedQuery.data.service || "geocoding",
        parsedQuery.data.period || "monthly",
      );
