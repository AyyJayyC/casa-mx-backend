...
const buyersRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get('/buyers', { onRequest: [verifyJWT] }, async (request, reply) => {
    try {
      ...
      return reply.code(500).send({ success: false, error: 'Failed to fetch buyers' });
    }
  });
  ...
  fastify.patch('/buyers/:id', { onRequest: [verifyJWT] }, async (request, reply) => {
    try {
      const user = (request as any).user;
      const { id } = request.params as { id: string };
      const existing = await fastify.prisma.buyer.findUnique({ where: { id } });
  ...
  fastify.delete('/buyers/:id', { onRequest: [verifyJWT] }, async (request, reply) => {
    try {
      const user = (request as any).user;
      const { id } = request.params as { id: string };
      const existing = await fastify.prisma.buyer.findUnique({ where: { id } });
      ...
      fastify.log.error(error);
      return reply.code(500).send({ success: false, error: 'Failed to delete buyer' });
    }
  });
};
