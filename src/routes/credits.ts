...
const creditsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get(
    ...
    async (request, reply) => {
      try {
        ...
        return reply
          .code(500)
          .send({ success: false, error: "Failed to fetch balance" });
      }
    },
  );
  ...
  fastify.get("/credits/packages", async (_request, reply) => {
    try {
      ...
      return reply
        .code(500)
        .send({ success: false, error: "Failed to fetch packages" });
    }
  });
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        ...
        return reply
          .code(500)
          .send(createServerErrorResponse("Failed to spend credit"));
      }
    },
  );
    ...
    async (request, reply) => {
      try {
        if (!env.STRIPE_SECRET_KEY) {
          return reply
            .code(503)
            .send({ success: false, error: "Payments not configured" });
        }
        const { packageId } = CreatePaymentIntentSchema.parse(request.body);
        const result = await creditsService.createPaymentIntent(
          request.user.id,
          packageId,
        );
        ...
        return reply
          .code(500)
          .send({ success: false, error: "Failed to create payment intent" });
      }
    },
  );
    ...
    async (request, reply) => {
      try {
        ...
        if (
          ...
          env.FULFILL_SECRET !== body.secret
        ) {
          return reply.code(404).send({ success: false, error: "Not found" });
        }
        const { stripePaymentIntentId, packageId } = FulfillPaymentSchema.parse(
          request.body,
        );
        const newBalance = await creditsService.fulfillPayment(
        ...
        return reply
          .code(500)
          .send({ success: false, error: "Failed to fulfill payment" });
      }
    },
  );
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        ...
        await fastify.prisma.creditPackage.updateMany({
          where: {},
          data: { active: false },
        });
        for (const pkg of packages) {
          const existing = await fastify.prisma.creditPackage.findFirst({
            where: { name: pkg.name },
          });
          if (existing) {
        ...
        return reply
          .code(500)
          .send({ success: false, error: "Failed to sync packages" });
      }
    },
  );
  ...
  fastify.post(
    ...
    async (request, reply) => {
      try {
        ...
        await creditsService.handleWebhook(
          rawBody,
          signature,
      ...
      try {
        ...
        if (transaction.type !== "purchase") {
          return reply
            ...
            .send({
              success: false,
              error: "Only purchase transactions can be invoiced",
            });
        }
        ...
        return reply
          .code(500)
          .send({ success: false, error: "Failed to generate invoice" });
      }
    },
  );
};
