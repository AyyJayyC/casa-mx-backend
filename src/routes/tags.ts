...
const tagsRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.get('/users/tags', { preHandler: [verifyJWT] }, async (request, reply) => {
  ...
  app.post('/users/tags', { preHandler: [verifyJWT] }, async (request, reply) => {
    const userId = (request as any).user?.id;
    if (!userId) return reply.code(401).send({ error: 'Unauthorized' });
    ...
    if (!['ciudad', 'colonia'].includes(tagType)) {
      return reply.code(400).send({ error: 'tagType must be ciudad or colonia' });
    }
    ...
    if (existing) {
      return reply.code(409).send({ error: 'Already subscribed to this tag' });
    }

    const sub = await prisma.tagSubscription.create({
      data: { userId, tagType, tagName, tagNormal, estado: estado || undefined },
    });
  ...
  app.delete('/users/tags/:id', { preHandler: [verifyJWT] }, async (request, reply) => {
    const userId = (request as any).user?.id;
    if (!userId) return reply.code(401).send({ error: 'Unauthorized' });
    ...
    if (sub.userId !== userId) return reply.code(403).send({ error: 'Forbidden' });
