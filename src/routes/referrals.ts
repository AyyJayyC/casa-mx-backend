...
const referralsRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.get('/referrals/my-code', { onRequest: [verifyJWT] }, async (request, reply) => {
    try {
      ...
      return reply.send({ success: true, data: { referralCode: user.referralCode } });
    } catch (error) {
      fastify.log.error(error);
      return reply.code(500).send({ success: false, error: 'Failed to get referral code' });
    }
  });

  // POST /referrals/click — log a click on a shared link (no auth needed)
  fastify.post<{ Body: Record<string, any> }>('/referrals/click', async (request, reply) => {
    try {
      ...
      fastify.log.error(error);
      return reply.code(500).send({ success: false, error: 'Failed to log click' });
    }
  });

  // GET /referrals/stats — current user's referral stats
  fastify.get('/referrals/stats', { onRequest: [verifyJWT] }, async (request, reply) => {
    try {
      const userId = request.user.id;
      ...
      if (!user?.referralCode) {
        return reply.send({
          success: true,
          data: { clicks: 0, signups: 0, conversionRate: 0, recentEvents: [] },
        });
      }
      ...
      fastify.log.error(error);
      return reply.code(500).send({ success: false, error: 'Failed to get referral stats' });
    }
  });
};
