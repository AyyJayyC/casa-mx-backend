...
export default async function carouselRoutes(app: FastifyInstance) {
  ...
  app.get('/admin/carousel', { onRequest: [requireAdmin] }, async (_req, reply) => {
    const slides = await app.prisma.carouselSlide.findMany({
      orderBy: { order: 'asc' },
    });
    return reply.send({ slides });
  ...
  app.delete('/admin/carousel/:id', { onRequest: [requireAdmin] }, async (req, reply) => {
    const { id } = req.params as { id: string };
  ...
  app.patch('/admin/carousel/reorder', { onRequest: [requireAdmin] }, async (req, reply) => {
