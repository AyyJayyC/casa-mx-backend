...
const contractsRoutes: FastifyPluginAsync = async (fastify) => {
   ...
   */
  fastify.get(
    "/contracts/rental/:applicationId",
    { onRequest: [verifyJWT] },
    async (request, reply) => {
      try {
        ...
        const pdfBuffer = await generateRentalContract(
          fastify.prisma,
          applicationId,
        );
        ...
        reply
          ...
          .header(
            "Content-Disposition",
            `inline; filename="contrato-arrendamiento-${applicationId.substring(0, 8)}.pdf"`,
          )
          .send(pdfBuffer);
      } catch (error: any) {
        ...
        return reply
          .code(500)
          .send({ success: false, error: "Failed to generate contract" });
      }
    },
  );
