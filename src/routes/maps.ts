...
const mapsRoutes: FastifyPluginAsync = async (fastify, opts) => {
  const mapServiceError = (err: unknown) => {
    const message =
      err instanceof Error ? err.message : String(err || "maps_failed");
    ...
    if (message.includes("Google Maps provider unavailable")) {
      return {
        ...
        payload: {
          ...
          message:
            "Google Maps is not configured. Set MAPS_API_KEY and ENABLE_BILLABLE_MAPS=true.",
        },
      };
    }
    ...
    if (message.includes("API key is invalid")) {
      return {
        ...
        payload: {
          ...
          message:
            "Google Maps rejected the configured API key. Replace MAPS_API_KEY with a valid server-side key.",
        },
      };
    }
    ...
    if (message.includes("no Mexico results")) {
      return {
        ...
        payload: {
          ...
          message:
            "No se encontró una dirección válida en México para la búsqueda proporcionada.",
        },
      };
    }
  ...
  fastify.post("/maps/geocode", async (request, reply) => {
    ...
    if (!parsedBody.success) {
      return reply.code(400).send({
        ...
        message:
          "address must be a non-empty string between 3 and 300 characters.",
  ...
  fastify.get("/maps/autocomplete", async (request, reply) => {
    ...
    if (!parsedQuery.success) {
      return reply.code(400).send({
        error: "invalid_request",
        message:
          "input must be a non-empty string between 3 and 120 characters.",
        details: parsedQuery.error.issues,
      });
    }

    try {
      const preds = await mapsService.autocomplete(parsedQuery.data.input, {
        userId: (request as any).user?.id,
        sessionToken: parsedQuery.data.sessionToken,
      });
