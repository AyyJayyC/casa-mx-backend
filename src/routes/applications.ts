    ...
    async (request, reply) => {
      try {
        ...
        const existingApplication =
          await fastify.prisma.rentalApplication.findFirst({
            where: {
        ...
        try {
          ...
          if (landlord) {
            await createNotification(
              ...
              `${tenant?.name ?? "Un inquilino"} envió una solicitud para "${property.title}".`,
              "application",
              application.id,
            );
    ...
    async (request, reply) => {
      try {
        ...
        if (input.status === "approved") {
          await fastify.prisma.rentalApplication.updateMany({
            ...
            data: {
              status: "rejected",
              landlordNote:
                "Another application was approved for this property",
            },
          });
        ...
        if (notifTitles[input.status]) {
          ...
          // Send email
          const applicantUser = await fastify.prisma.user.findUnique({
