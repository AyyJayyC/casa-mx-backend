  ...
  fastify.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      if (property.verificationStatus === "verified") {
        return reply
          .code(400)
          .send({ success: false, error: "Property is already verified" });
      }

      if (!isS3Configured()) {
        return reply
          .code(503)
          .send({ success: false, error: "Document storage not configured" });
      }
      ...
      if (!VALID_DOC_TYPES.has(documentType)) {
        return reply
          ...
          .send({
            success: false,
            error: `Invalid documentType. Valid: ${[...VALID_DOC_TYPES].join(", ")}`,
          });
      }
      ...
      try {
        ...
        fastify.log.error(
          { err, propertyId, userId, fileName: filePart.filename },
          "S3 upload failed for property document",
        );
    ...
    async (request, reply) => {
      ...
      if (property.verificationStatus === "verified") {
        return reply
          ...
          .send({
            success: false,
            error: "Cannot remove documents from a verified property",
          });
      }
      ...
      if (!doc)
        return reply
          .code(404)
          .send({ success: false, error: "Document not found" });
      ...
      const remaining = await fastify.prisma.propertyDocument.count({
        where: { propertyId },
      });
      if (remaining === 0) {
