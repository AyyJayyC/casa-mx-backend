...
async function main() {
  ...
  await record('GET /health returns ok', async () => {
    ...
    if (data.checks.email === 'degraded') throw new Error(`Email degraded: ${data.checks.email}`);
  });
  ...
  await record('Admin user can log in', async () => {
    ...
    // Extract token from set-cookie (or from response directly)
  ...
  // 3. Email health
  await record('Email service verified domain', async () => {
    const data = await fetchJson(`${BACKEND_URL}/health`);
    if (data.checks.email !== 'ok') throw new Error(`Email check: ${data.checks.email}`);
  });
