...
async function registerAndLogin(name, roles) {
  ...
  const reg = await api('POST', '/auth/register', null, { name, email, password, roles });
...
async function setup() {
  ...
  const adminLogin = await api('POST', '/auth/login', null, { email: 'admin@casamx.local', password: 'admin123' });
...
async function flowASale() {
  ...
  const prop = await api('POST', '/properties', sellerToken, {
    ...
    price: 3500000,
    inventoryNotes: 'Check-in E2E: todas las ventanas funcionan, pintura fresca, 2 climas nuevos.',
  });
  if (prop.status === 403) {
    console.log('  ⚠ Seller role not approved yet — property may not be created');
  ...
  assert(req.status === 201 || req.status === 409, `Contact request → ${req.status}`);
  if (req.data.data) requestId = req.data.data.id;
  console.log(`  Request: ${requestId || 'already exists'}`);
  
  // A3. Buyer submits offer
  const offer = await api('POST', `/properties/${salePropertyId}/offers`, buyerToken, {
    offerAmount: 3400000,
    financing: 'bankLoan',
...
async function flowBRental() {
  ...
  const prop = await api('POST', '/properties', sellerToken, {
    ...
    amenities: ['Refrigerador', 'Estufa', 'Lavadora', 'Minisplits', 'Calentador de agua'],
    inventoryNotes: 'Check-in E2E: Refrigerador con pequeño rayon en puerta. Pintura fresca en sala. Minisplits funcionando. Persianas completas.',
  });
  ...
  const app = await api('POST', '/applications', buyerToken, {
    ...
    desiredMoveInDate: new Date(Date.now() + 14 * 86400000).toISOString().split('T')[0],
    desiredLeaseTerm: 12,
    numberOfOccupants: 2,
    reference1Name: 'Juan Perez',
    reference1Phone: '5598765432',
    messageToLandlord: 'Solicito el departamento, puedo depositar inmediatamente',
  });
  ...
  const review = await api('PATCH', `/applications/${applicationId}`, sellerToken, {
    status: 'under_review',
    landlordNote: 'Revisando referencias',
  });
  assert(review.status === 200, `Review application → ${review.status}`);
  
  const approve = await api('PATCH', `/applications/${applicationId}`, sellerToken, {
    status: 'approved',
  ...
  assert(pdfBuffer.byteLength > 800, `Contract size ${pdfBuffer.byteLength} bytes (min 800)`);
  
  // Quick text check: does the PDF contain key terms?
  const text = new TextDecoder().decode(pdfBuffer.slice(0, 3000));
  const checks = ['NOM-247', 'INVENTARIO', 'ARRENDAMIENTO', 'CasaMX', 'ARRENDADOR'];
  for (const c of checks) {
    assert(text.includes(c), `Contract contains "${c}"`);
  }
  console.log(`  Contract PDF: ${(pdfBuffer.byteLength/1024).toFixed(1)} KB — NOM-247 + Inventory verified`);
...
async function flowCEdgeCases() {
  ...
  assert(matches >= 1, `Colonia "roma" matches ${matches} properties (need >=1)`);
  
  // C3. Ownership protection
  const steal = await api('PATCH', `/properties/${rentalPropertyId}`, buyerToken, {
    price: 1,
  });
  ...
  if (salePropertyId) {
    const sold = await api('POST', `/properties/${salePropertyId}/offers`, buyerToken, {
      offerAmount: 1000000,
      financing: 'cash',
      buyerName: 'Test',
      buyerEmail: 't@t.com',
