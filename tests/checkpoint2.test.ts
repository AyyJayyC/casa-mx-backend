...
describe("Checkpoint 2 - Authentication & Admin Bootstrap", () => {
  ...
  it("should persist active refresh token jti in token store", async () => {
    ...
    const loginBody = loginResponse.json() as any;
    const decodedRefresh = app.jwt.decode(
      getCookie(loginResponse, "refreshToken") ?? loginBody.refreshToken,
    ) as any;
  ...
  it("should revoke old refresh jti and rotate active jti on refresh", async () => {
    ...
    expect(refreshResponse.statusCode).toBe(200);
    const secondDecoded = app.jwt.decode(
      getCookie(refreshResponse, "refreshToken"),
    ) as any;

    const wasRevoked = await refreshTokenStoreService.isJtiRevoked(
      firstDecoded.jti,
    );
    const activeJti = await refreshTokenStoreService.getActiveJtiForUser(
      loginBody.user.id,
  ...
  it("should have admin user created via seed", async () => {
    ...
    expect(admin?.roles.find((r) => r.role.name === "admin")?.status).toBe(
      "approved",
    );
  });
