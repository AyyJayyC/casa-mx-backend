...
describe('Forgot Password & Reset Password Flow', () => {
  ...
  describe('POST /auth/forgot-password', () => {
    it('should return 200 for existing user and generate reset token', async () => {
      ...
      expect(new Date(user!.passwordResetTokenExpiresAt!).getTime()).toBeGreaterThan(Date.now());
    });
  ...
  describe('POST /auth/reset-password', () => {
    it('should reset password with valid token', async () => {
      ...
      await app.inject({
        method: 'POST',
        url: '/auth/register',
        payload: { email, name: 'Reset User', password: oldPassword, roles: ['buyer'] },
      });
    ...
    it('should reject expired token', async () => {
      ...
      await app.inject({
        method: 'POST',
        url: '/auth/register',
        payload: { email, name: 'Expired User', password: 'Password1', roles: ['buyer'] },
      });
      ...
      await app.prisma.user.update({
        where: { email },
        data: { passwordResetToken: token, passwordResetTokenExpiresAt: expiredAt },
      });
    ...
    it('should reject invalid token', async () => {
      const response = await app.inject({
        method: 'POST',
        url: '/auth/reset-password',
        payload: { token: crypto.randomBytes(32).toString('hex'), password: 'NewPass1' },
      });
    ...
    it('should validate password strength', async () => {
      ...
      await app.inject({
        method: 'POST',
        url: '/auth/register',
        payload: { email, name: 'Weak Pass', password: 'Password1', roles: ['buyer'] },
      });

      const token = crypto.randomBytes(32).toString('hex');
      await app.prisma.user.update({
        where: { email },
        data: { passwordResetToken: token, passwordResetTokenExpiresAt: new Date(Date.now() + 3600000) },
      });
    ...
    it('should clear lockout state after successful reset', async () => {
      ...
      await app.inject({
        method: 'POST',
        url: '/auth/register',
        payload: { email, name: 'Lockout User', password: 'Password1', roles: ['buyer'] },
      });
