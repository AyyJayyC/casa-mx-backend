...
describe("Integrity Check - Adversarial Tests", () => {
  beforeAll(async () => {
    ...
    adminToken =
      getCookie(adminLogin, "accessToken") ?? (adminLogin.json() as any).token;
    ...
    userToken =
      getCookie(userLogin, "accessToken") ?? (userLogin.json() as any).token;
  ...
  describe("C - State Integrity: Database State Must Prevent Invalid Operations", () => {
    ...
    it('ADVERSARIAL: Approved application MUST set property status to "rented"', async () => {
      ...
      const landlordToken =
        ...
        (landlordLogin.json() as any).token;
