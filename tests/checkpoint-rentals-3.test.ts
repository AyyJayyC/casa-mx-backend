...
describe("Checkpoint 3 - Rental Application Endpoints", () => {
  ...
  beforeAll(async () => {
    ...
    tenantToken =
      getCookie(tenantLogin, "accessToken") ??
      JSON.parse(tenantLogin.body).token;
    ...
    landlordToken =
      getCookie(landlordLogin, "accessToken") ??
      JSON.parse(landlordLogin.body).token;
  ...
  describe("GET /applications - View own applications", () => {
    ...
    it("should filter applications by status", async () => {
      ...
      const body = JSON.parse(response.body);
      expect(body.success).toBe(true);
      expect(body.data.every((app: any) => app.status === "pending")).toBe(
        true,
      );
    });
  ...
  describe("GET /applications/property/:propertyId - Landlord views applications", () => {
    ...
    it("should reject landlord viewing applications for property they don't own", async () => {
      ...
      const otherLandlordToken =
        getCookie(otherLandlordLogin, "accessToken") ??
        JSON.parse(otherLandlordLogin.body).token;
  ...
  describe("PATCH /applications/:id - Update application status", () => {
    ...
    it("should auto-reject other applications when one is approved", async () => {
      ...
      if (!app1Body.data) {
        throw new Error(
          `Application creation failed: ${JSON.stringify(app1Body)}`,
        );
      }
      ...
      if (!registerBody.user) {
        throw new Error(
          `Tenant 2 registration failed: ${JSON.stringify(registerBody)}`,
        );
      }
      ...
      const tenant2Token =
        getCookie(tenant2Login, "accessToken") ??
        JSON.parse(tenant2Login.body).token;
    ...
    it("should reject non-landlord updating application", async () => {
      ...
      if (!createBody.data) {
        throw new Error(
          `Application creation failed: ${JSON.stringify(createBody)}`,
        );
      }
