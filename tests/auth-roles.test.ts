...
describe('Auth Roles - Admin auto-approval & self-healing', () => {
  ...
  describe('getInitialRoleStatus (via register)', () => {
    it('should auto-approve all roles for ADMIN_EMAIL user, including admin', async () => {
      ...
      if (existing) {
        await app.prisma.userRole.deleteMany({ where: { userId: existing.id } });
      ...
      const response = await app.inject({
        method: 'POST',
        url: '/auth/register',
        payload: { email, name: 'Admin User', password, roles: ['buyer', 'admin'] },
      });
    ...
    it('should auto-approve buyer/tenant for any user', async () => {
      ...
      const response = await app.inject({
        method: 'POST',
        url: '/auth/register',
        payload: { email, name: 'Auto Roles', password: 'Password1', roles: ['buyer', 'tenant'] },
      });

      expect(response.statusCode).toBe(201);
      const body = response.json() as any;
      const buyerRole = body.user.roles.find((r: any) => r.roleName === 'buyer');
      const tenantRole = body.user.roles.find((r: any) => r.roleName === 'tenant');
      expect(buyerRole.status).toBe('approved');
    ...
    it('should set seller/landlord/wholesaler pending for regular user', async () => {
      ...
      const response = await app.inject({
        method: 'POST',
        url: '/auth/register',
        payload: { email, name: 'Pending Roles', password: 'Password1', roles: ['seller', 'landlord', 'wholesaler'] },
      });
  ...
  describe('Login self-healing (pending admin -> approved)', () => {
    it('should auto-approve pending admin role on login for ADMIN_EMAIL user', async () => {
      ...
      const adminRole = await app.prisma.role.findUnique({ where: { name: 'admin' } });
      ...
      const adminInResponse = loginBody.user.roles.find((r: any) => r.roleName === 'admin');
